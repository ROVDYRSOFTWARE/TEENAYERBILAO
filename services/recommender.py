...
def apply_preferences(token: str, categorias: list[str], barrios: list[str], franjas: list[str], presupuesto: list[str]) -> None:
    profiles = data_store.load_profiles()
    profile = _ensure_profile(profiles, token)
    for value in categorias: _bump(profile["gustos"], value, 3.0)
    for value in barrios: _bump(profile["barrios"], value, 2.0)
    for value in franjas: _bump(profile["franjas"], value, 2.0)
    for value in presupuesto: _bump(profile["presupuesto"], value, 2.0)
    profile["updated_at"] = data_store.now_iso()
    data_store.save_profiles(profiles)
    data_store.append_interaction({
        "ts": data_store.now_iso(),
...
def get_profile(token: str) -> dict:
    profiles = data_store.load_profiles()
    return profiles.get(token, {"gustos": {}, "barrios": {}, "franjas": {}, "presupuesto": {}, "tags": {}, "acciones": 0})
...
def score_item(profile: dict, item: dict, popularity_counter: Counter | None = None) -> float:
    ...
    if item.get("_entity_type") == "evento":
        ...
        try:
            ...
            days_delta = (dt.date() - datetime.now().date()).days
            if 0 <= days_delta <= 7: score += 3
            elif days_delta < 0: score -= 2
        except Exception:
            pass
...
def plan_hoy(token: str, events: list[dict], places: list[dict]) -> dict:
    ranked = rank_items(token, events, places)
    principal = next((x for x in ranked if x.get("_entity_type") == "evento"), None)
    barrio_ref = principal.get("barrio") if principal else None
    compatibles = [x for x in ranked if x.get("_entity_type") == "lugar" and (not barrio_ref or x.get("barrio") == barrio_ref)]
    comida = next((x for x in compatibles if x.get("categoria") in {"comida", "cafe", "bubble-tea", "restaurante"}), None)
    extra = next((x for x in compatibles if x.get("franja") in {"tarde", "noche"} and x != comida), None)
    return {"principal": principal, "comida": comida, "extra": extra, "ranked": ranked[:12]}
